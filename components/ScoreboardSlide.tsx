  ...
  return (
    <div className="flex-grow bg-slate-100 flex flex-col p-4 md:p-8 relative overflow-hidden">
        ...
        <div className="flex justify-between items-end mb-6 border-b-4 border-news-red pb-2">
             <div>
                <span className="bg-news-red text-white px-3 py-1 text-sm font-bold uppercase tracking-wider">Official Standings</span>
        ...
        <AnimatePresence>
          {sortedMadrasas.map((item, index) => {
            ...
            return (
              ...
              >
                {/* Rank Badge */}
                <div className={`absolute top-0 right-0 p-2 font-display font-bold text-4xl opacity-20 ${isFirst ? 'text-white' : 'text-black'}`}>
                    #{index + 1}
                </div>

                {/* Icons for top ranks */}
                {index === 0 && <Trophy className="absolute bottom-4 right-4 text-news-gold opacity-20" size={120} />}
                {index === 1 && <Medal className="absolute bottom-4 right-4 text-gray-400 opacity-20" size={80} />}
                {index === 2 && <Medal className="absolute bottom-4 right-4 text-amber-600 opacity-20" size={80} />}

                <div className="z-10">
                    {isFirst && <div className="text-news-gold font-bold uppercase tracking-widest text-sm mb-2">Current Champion</div>}
                    <h3 className={`${isFirst ? 'text-4xl md:text-5xl' : 'text-2xl'} font-display font-bold leading-tight ${textColor}`}>
                        {item.name}
                    </h3>
                </div>

                <div className="z-10 mt-4 flex items-baseline gap-2">
                    <span className={`font-mono font-bold ${isFirst ? 'text-6xl md:text-7xl' : 'text-4xl'} ${scoreColor}`}>
                        {item.score}
                    </span>
                ...
                {!isFirst && (
                    <div className="w-full bg-gray-200 h-2 mt-4 rounded-full overflow-hidden">
                        <div 
                            ...
                            style={{ width: `${(item.score / sortedMadrasas[0].score) * 100}%` }}
                        ></div>
                    </div>
                )}
              </motion.div>
