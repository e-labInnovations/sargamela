  ...
  return (
    <div className="flex-grow bg-white flex flex-col p-2 relative overflow-hidden">
       ...
       <div className="flex items-center gap-4 mb-2 border-b-2 border-news-black pb-1 shrink-0">
            ...
            <h2 className="text-3xl font-display font-bold text-news-red uppercase truncate">
                {data.title}
            </h2>
       </div>
       ...
       <div className="flex-grow flex flex-col bg-slate-50 rounded shadow-inner border border-slate-200 overflow-hidden relative">
          ...
          <div className="flex border-b-2 border-news-dark bg-news-dark text-white shrink-0 z-20">
              ...
              <div className={`${nameColWidth} shrink-0 p-2 font-display text-lg uppercase tracking-wider border-r border-red-800 flex items-end pb-2 bg-news-dark z-30 shadow-[4px_0_5px_rgba(0,0,0,0.2)]`}>
                  Madrasa
              </div>
              ...
              <div className="flex-grow overflow-hidden relative">
                   <motion.div 
                        ...
                        transition={{ duration: scrollWidth > 0 ? 20 : 0, ease: "linear", delay: 1 }}
                        className="flex h-48 items-end"
                   >
                        {data.headers.map((h, i) => (
                            <div key={i} className={`${colWidth} shrink-0 pb-2 font-sans text-xs font-bold uppercase tracking-tight border-r border-red-800 text-center flex items-end justify-center`}>
                                <div className="rotate-180 [writing-mode:vertical-rl] whitespace-nowrap text-left w-full h-full flex items-center px-1">
                                  {h}
                                </div>
                            </div>
                        ))}
          ...
          <div className="flex-grow overflow-hidden flex relative">
              ...
              <div className={`${nameColWidth} shrink-0 bg-white border-r border-slate-300 z-10 shadow-[4px_0_5px_rgba(0,0,0,0.1)]`}>
                  {data.rows.map((row, idx) => (
                      <div 
                        key={idx} 
                        className={`${rowHeight} flex items-center px-3 font-bold text-base text-news-black border-b border-slate-200 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}
                      >
                          <span className="whitespace-nowrap overflow-visible">{row.category}</span>
                      </div>
                  ))}
              </div>
              ...
              <div ref={contentRef} className="flex-grow overflow-hidden relative bg-slate-50">
                  ...
                  >
                      {data.rows.map((row, idx) => (
                          <div key={idx} className={`${rowHeight} flex border-b border-slate-200 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                                {row.values.map((v, i) => (
                                    <div key={i} className={`${colWidth} shrink-0 flex items-center justify-center text-lg font-mono text-slate-700 border-r border-slate-200`}>
                                        {v > 0 ? v : <span className="text-gray-200 text-xs">.</span>}
                                    </div>
                                ))}
                                <div className="w-24 shrink-0 flex items-center justify-center text-xl font-bold bg-red-50 text-news-red font-mono border-l border-slate-300">
                                    {row.total}
                                </div>
                          </div>
                      ))}
